...
export const PlayerFilter: React.FC<PlayerFilterProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 w-full flex flex-col p-6 animate-in">
      ...
      <div className="rounded-lg border border-slate-200 dark:border-slate-800 flex flex-col max-h-64 overflow-y-auto bg-white dark:bg-slate-900 shadow-inner">
        {players.map((p, i) => {
            ...
            >
              ...
              <span className={`truncate text-xs ${isSelected ? 'font-bold' : ''}`}>
                ...
                {showRank && p.rank !== undefined && (
                  <span className="text-slate-400 dark:text-slate-500 ml-1 font-normal">({p.rank}{MESSAGES.ANALYSIS_RANK_SUFFIX})</span>
                )}
              </span>
            </label>
          );
      ...
      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-2 text-center italic">{MESSAGES.PLAYER_FILTER_SYNC_NOTE}</p>
    </div>
  );
};
